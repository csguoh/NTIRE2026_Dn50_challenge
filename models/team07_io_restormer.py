...
from models.team07_I2WM_JNU.basicsr.archs.restormer_arch import Restormer, RestormerLocal
...
def run(model, data_path, save_path, tile, device):
    ...
    for i, img_lr in enumerate(input_img_list):
        ...
        # (1) img_lr
        # --------------------------------
        img_hr = img_lr.replace('denoising_val_input', 'denoising_val_gt')
        img_hr = img_hr.replace('DIV2K_test_noise50', 'DIV2K_test_HR')
        ...
        img_sr = forward(img_lr, model, tile=2000, tile_overlap=1600, scale=1)
...
def main(model_dir, input_path, output_path, device=None):
    utils_logger.logger_info("NTIRE2024-ImageSRx4", log_path="NTIRE2024-ImageSRx4-MambaIRv2.log")
